...
impl MlManager {
    ...
    pub fn generate_zone_suggestions(
        &self,
        zones: &[crate::geofencing::GeofenceZone],
    ) -> Vec<crate::geofencing::ipc::ZoneSuggestion> {
        use crate::geofencing::ipc::{SuggestionEvidence, SuggestionPriority, ZoneSuggestion};
    ...
    pub fn record_zone_change(&mut self, from_zone_id: Option<&str>, to_zone_id: &str) {
        debug!(
            "Recording zone change from {:?} to {:?}",
            from_zone_id, to_zone_id
        );

        // Update usage patterns with basic zone switch action
        let user_action =
            crate::ml::usage_patterns::UserAction::ConnectWifi(format!("zone-{}", to_zone_id));
    ...
    pub fn record_scan_performance(
        &mut self,
        scan_duration: std::time::Duration,
        scan_interval: std::time::Duration,
    ) {
        debug!(
            "Recording scan performance: duration={:?}, interval={:?}",
        ...
        self.performance_tracker
            .record_metrics(&connection_id, metrics);
    }

    /// Get ML-enhanced adaptive scan interval based on comprehensive performance metrics
    pub fn get_adaptive_scan_interval(
        &self,
        base_interval: std::time::Duration,
        recent_changes: u32,
        current_context: &NetworkContext,
    ) -> std::time::Duration {
        debug!(
            "📊 Calculating adaptive scan interval: base={:?}, recent_changes={}, context={:?}",
    ...
    pub fn record_enhanced_scan_performance(
        &mut self,
        scan_duration: std::time::Duration,
        scan_interval: std::time::Duration,
        ...
        self.performance_tracker
            .record_metrics(&connection_id, enhanced_metrics);

        // Also record this as a user action for pattern learning
        let scan_action = crate::ml::usage_patterns::UserAction::ConnectWifi(format!(
            "scan_performance_{}_{}zones",
        ...
        info!(
            "📈 Enhanced scan metrics recorded: {:?} -> {} zones, changed={}, confidence={:.1}%",
            scan_duration,
            zones_detected,
            zone_changed,
            confidence_score * 100.0
    ...
    pub fn get_ml_metrics(
        &self,
        suggestions_generated: u32,
        auto_suggestions_processed: u32,
    ...
    pub fn calculate_zone_confidence(
        &self,
        zone_id: &str,
        current_fingerprint: &crate::geofencing::LocationFingerprint,
    ) -> f64 {
    ...
    pub fn track_prediction_accuracy(
        ...
        debug!(
            "📊 Tracking prediction accuracy: predicted={}, actual={}, confidence={:.3}",
        ...
        let user_action = if is_correct {
            crate::ml::usage_patterns::UserAction::ConnectWifi(format!(
                "correct_prediction_{}",
                actual_zone_id
            ))
        } else {
            crate::ml::usage_patterns::UserAction::ConnectWifi(format!(
                "incorrect_prediction_{}_{}",
                predicted_zone_id, actual_zone_id
            ))
        };
        ...
        let prediction_metrics = NetworkMetrics {
            ...
            jitter_ms: if is_correct {
                confidence as f32
            } else {
                (1.0 - confidence) as f32
            },
        ...
        self.performance_tracker
            .record_metrics(&connection_id, prediction_metrics);

        info!(
            "📊 Prediction accuracy recorded: {} (confidence: {:.1}%)",
            if is_correct {
                "✅ CORRECT"
            } else {
                "❌ INCORRECT"
    ...
    pub fn get_confidence_threshold(&self, zone_id: &str) -> f64 {
        ...
        debug!(
            "🎯 Dynamic confidence threshold for {}: base={:.3}, network_adj={:.3}, final={:.3}",
    ...
    pub fn should_auto_accept_suggestion(
        &self,
        suggestion: &crate::geofencing::ipc::ZoneSuggestion,
        current_context: &NetworkContext,
    ) -> bool {
        debug!(
            "🤖 Evaluating auto-acceptance for suggestion: {} (confidence: {:.3})",
    ...
    pub fn generate_enhanced_zone_suggestions(
        &self,
        zones: &[crate::geofencing::GeofenceZone],
    ) -> Vec<(crate::geofencing::ipc::ZoneSuggestion, bool)> {
        ...
        basic_suggestions
            ...
            .map(|suggestion| {
                let should_auto_accept =
                    self.should_auto_accept_suggestion(&suggestion, &current_context);
    ...
    pub fn process_auto_suggestions(
        &mut self,
        zones: &[crate::geofencing::GeofenceZone],
    ) -> Vec<AutoSuggestionResult> {
        ...
        for (suggestion, should_auto_accept) in enhanced_suggestions {
            if should_auto_accept {
                ...
                results.push(AutoSuggestionResult {
                    ...
                    reasoning: format!(
                        "Auto-created due to high confidence ({:.1}%) and {} priority",
                ...
                info!(
                    "🤖 ✨ Auto-suggestion processed: creating zone '{}' with {:.1}% confidence",
    ...
    pub fn process_zone_transition_with_smoothing(
        &mut self,
        ...
        self.usage_learner
            .record_action(transition_action, current_context);
        decision
    }
...
impl ZoneTransitionSmoother {
    ...
    pub fn process_zone_transition(
        &mut self,
        candidate_zone_id: &str,
        confidence: f64,
        current_context: &NetworkContext,
        ...
        debug!(
            "🌊 Processing zone transition smoothing: current={:?}, candidate={}, confidence={:.3}",
            ...
            || self.current_zone_id.as_deref() == Some(candidate_zone_id)
        {
            if confidence >= 0.8 {
                ...
                return ZoneTransitionDecision::Reject(
                    "Confidence too low for initial/same zone".to_string(),
                );
            }
        }
    ...
    fn evaluate_pending_transition(
        &mut self,
        confidence: f64,
        current_context: &NetworkContext,
        ...
        let duration_factor =
            (pending_duration as f64 / self.smoothing_window_seconds as f64).min(1.0);
        let base_threshold = 0.75;
    ...
    fn evaluate_new_transition(
        ...
        if self.detect_oscillation(candidate_zone_id, now) {
            debug!(
                "🌊 🔄 Detected oscillation pattern for zone: {}, applying damping",
                candidate_zone_id
            );
    ...
    fn detect_oscillation(
        ...
        if self.current_zone_id.is_some() {
            ...
            let recent_departures = self
                .transition_history
                .iter()
                .rev()
                .take(5) // Last 5 transitions
                .filter(|t| {
                    t.from_zone_id.as_deref() == Some(candidate_zone_id)
                        && now.signed_duration_since(t.timestamp).num_seconds() < 300
                }) // Last 5 minutes
                .count();

            if recent_departures >= 2 {
                debug!(
                    "🌊 🔄 Detected rapid return pattern: {} was recently departed {} times",
                    candidate_zone_id, recent_departures
                );
                return true;
            }
        }
    ...
    pub fn get_pending_transition(&self) -> Option<(String, u64, f64)> {
        if let (Some(pending_zone), Some(pending_since)) =
            (&self.pending_zone_id, &self.pending_since)
        {
