...
pub fn prompt_for_text(description: &str) -> Result<String, Box<dyn std::error::Error>> {
...

    {
        ...
        write!(stdin, "SETDESC {description}\nGETPIN\n")?;
    }
    ...
    let value = value_line.trim_start_matches("D ").trim().to_string();

    Ok(value)
}

pub fn prompt_for_ssid() -> Result<String, Box<dyn std::error::Error>> {
    prompt_for_text("Enter SSID")
}

pub fn prompt_for_password(ssid: &str) -> Result<String, Box<dyn std::error::Error>> {
    prompt_for_text(&format!("Enter {ssid} password"))
}

pub fn prompt_for_visible_text(description: &str) -> Result<String, Box<dyn std::error::Error>> {
    if crate::command::is_command_installed("dmenu") {
    ...
    if crate::command::is_command_installed("rofi") {
