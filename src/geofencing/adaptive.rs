...
impl AdaptiveScanner {
    ...
    pub fn new(config: ScanFrequency) -> Self {
        debug!(
            "Creating adaptive scanner with base interval: {:?}",
    ...
    pub async fn calculate_optimal_interval(&mut self) -> Duration {
        ...
        debug!(
            "Current context - Movement: {:?}, Power: {:?}, Zone stability: {}",
        ...
        if self.should_update_interval(new_interval) {
            debug!(
                "Updating scan interval from {:?} to {:?} (multiplier: {:.2})",
    ...
    pub fn update_movement_detection(&mut self, fingerprint: &LocationFingerprint) {
        debug!(
            "Updating movement detection with fingerprint confidence: {:.2}",
    ...
    pub fn update_zone_stability(&mut self, zone_id: Option<&str>, confidence: f64) {
        debug!(
            "Updating zone stability - Zone: {:?}, Confidence: {:.2}",
            zone_id, confidence
        );
        ...
        if let Some(_current_zone) = zone_id {
            ...
            if self.zone_stability.time_in_zone == ChronoDuration::zero() {
                ...
                // Update time in zone
                self.zone_stability.time_in_zone = now - (now - self.zone_stability.time_in_zone);
            }

            self.zone_stability.confidence_score = confidence;
            self.zone_stability.is_stable = confidence > 0.8
        ...
        debug!(
            "Zone stability updated - Stable: {}, Time in zone: {}min, Confidence: {:.2}",
    ...
    pub fn record_scan_event(
        ...
        debug!(
            "Recording scan event: duration={:?}, confidence={:.2}, zone={:?}",
...
impl MovementDetector {
    ...
    fn update(&mut self, fingerprint: &LocationFingerprint) {
        debug!(
            "Updating movement detector with {} WiFi networks",
        ...
        if self.fingerprint_history.len() >= 2 {
            ...
            debug!(
                "Movement analysis - Recent similarity: {:.2}, State: {:?}",
...
impl PowerMonitor {
    ...
    async fn update_power_state(&mut self) {
        ...
        #[cfg(not(feature = "geofencing"))]
        {
            ...
            if let Ok(info) = self.read_battery_info_fallback().await {
                ...
                if old_state != self.current_state {
                    info!(
                        "Power state changed from {:?} to {:?} (fallback)",
                        old_state, self.current_state
                    );
                }
            } else {
    ...
    async fn read_battery_info_fallback(&self) -> Result<BatteryInfo> {
        ...
        let ac_connected = if let Ok(content) =
            fs::read_to_string("/sys/class/power_supply/ADP1/online").await
        {
            content.trim() == "1"
        } else if let Ok(content) =
            fs::read_to_string("/sys/class/power_supply/macsmc-ac/online").await
        {
            content.trim() == "1"
