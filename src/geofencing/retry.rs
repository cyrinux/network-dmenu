...
impl RetryManager {
    ...
    pub fn new(config: RetryConfig) -> Self {
        debug!("Creating retry manager with config: max_retries={}, base_delay={:?}", 
    ...
    async fn execute_action(&self, action: &RetryableAction) -> Result<()> {
        ...
        match action {
            RetryableAction::WiFiConnection(ssid) => {
                ...
                if crate::command::is_command_installed("nmcli") {
                    let result = command_runner
                        .run_command("nmcli", &["device", "wifi", "connect", ssid]);
                    ...
                    match result {
                        ...
                        Ok(output) => {
                            ...
                            Err(GeofenceError::ActionExecution(format!(
                                "WiFi connection failed: {}", stderr
                            )))
                        }
                        Err(e) => Err(GeofenceError::ActionExecution(format!(
                            "Failed to execute nmcli: {}", e
                        ))),
                    }
                } else {
            ...
            RetryableAction::VpnConnection(vpn_name) => {
                ...
                if crate::command::is_command_installed("nmcli") {
                    let result = command_runner
                        .run_command("nmcli", &["connection", "up", vpn_name]);
                                ...
                                if line.contains(device_name) {
                                    if let Some(address_start) = line.find("Device ") {
                                        if let Some(address_end) = line[address_start + 7..].find(' ') {
                                            device_address = Some(
                                                line[address_start + 7..address_start + 7 + address_end]
                                                    .to_string(),
                                            );
                                            break;
                                        }
                            ...
                            if let Some(address) = device_address {
                                ...
                                match connect_result {
                                    ...
                                    Ok(output) => {
                                        let stderr = String::from_utf8_lossy(&output.stderr);
                                        Err(GeofenceError::ActionExecution(format!(
                                            "Bluetooth connection failed: {}", stderr
                                        )))
                                    }
                                    Err(e) => Err(GeofenceError::ActionExecution(format!(
                                        "Failed to execute bluetoothctl connect: {}", e
                                    ))),
                                }
                            } else {
                                Err(GeofenceError::ActionExecution(format!(
                                    "Bluetooth device '{}' not found", device_name
                                )))
                            }
                        }
                        Ok(output) => {
                            let stderr = String::from_utf8_lossy(&output.stderr);
                            Err(GeofenceError::ActionExecution(format!(
                                "Failed to list Bluetooth devices: {}", stderr
                            )))
                        }
                        Err(e) => Err(GeofenceError::ActionExecution(format!(
                            "Failed to execute bluetoothctl devices: {}", e
                        ))),
                    }
                } else {
                    Err(GeofenceError::ActionExecution(
            ...
            RetryableAction::TailscaleExitNode(exit_node) => {
                debug!("Executing Tailscale exit node configuration: '{}'", exit_node);
                ...
                if crate::command::is_command_installed("tailscale") {
                    ...
                    match result {
                        ...
                        Ok(output) => {
                            let stderr = String::from_utf8_lossy(&output.stderr);
                            Err(GeofenceError::ActionExecution(format!(
                                "Tailscale exit node configuration failed: {}", stderr
                            )))
                        }
                        Err(e) => Err(GeofenceError::ActionExecution(format!(
            ...
            RetryableAction::CustomCommand(command) => {
                ...
                if !self.is_safe_command(command) {
                    return Err(GeofenceError::ActionExecution(format!(
                        "Command failed security check: {}", command
                    )));
                }
    ...
    fn is_safe_command(&self, command: &str) -> bool {
        ...
        let dangerous_patterns = [
            ...
            "rm -rf", "rm -r", "sudo rm", "format", "mkfs", "dd if=", "fdisk", "parted",
            "> /dev/", "truncate", "shred",
            // System control
            "shutdown", "reboot", "halt", "poweroff", "systemctl reboot", "systemctl poweroff",
            // Network security
            "iptables -F", "ufw --force", "ufw disable", "systemctl stop firewalld",
            // Permissions
            "chmod 777", "chmod -R 777", "chown -R root", "chmod u+s",
            // Network access
            "curl", "wget", "nc ", "netcat", "telnet", "ssh ", "scp ", "rsync",
            // Code execution
            "python -c", "perl -e", "ruby -e", "node -e", "eval", "exec", "`", "$(",
            // User management  
            "passwd", "su ", "sudo su", "usermod", "userdel", "useradd",
            // System files
            "/etc/shadow", "/etc/passwd", "/etc/sudoers", "crontab",
            // Sensitive directories
            "/var/", "/etc/", "/root/", "/boot/", "/sys/", "/proc/kernel",
            // Package management
            "apt install", "yum install", "dnf install", "pacman -S", "pkg install",
        ...
        for pattern in &dangerous_patterns {
            if command_lower.contains(pattern) {
                debug!("Command blocked by dangerous pattern '{}': {}", pattern, command);
                return false;
            }
        }
        ...
        let safe_prefixes = [
            "systemctl --user start", "systemctl --user stop", "systemctl --user restart",
            "systemctl --user reload", "systemctl --user enable", "systemctl --user disable",
    ...
    pub async fn process_retries(&mut self) -> Vec<RetryStatus> {
        ...
        debug!("Processing {} actions in retry queue", self.failed_actions.len());
        ...
        while let Some(mut failed_action) = self.failed_actions.pop_front() {
            if failed_action.next_retry <= now {
                ...
                let status = self.execute_with_retry(failed_action.action.clone(), &context).await;
                ...
                match status {
                    ...
                    RetryStatus::Failed(_) => {
                        ...
                        if failed_action.attempt_count < self.config.max_retries {
                            ...
                            failed_action.next_retry = now + ChronoDuration::from_std(delay).unwrap_or(ChronoDuration::seconds(60));
        ...
        debug!("Retry processing complete. {} actions remain in queue", self.failed_actions.len());
    ...
    fn count_zone_actions(&self, actions: &ZoneActions) -> usize {
        let mut count = 0;
        if actions.wifi.is_some() { count += 1; }
        if actions.vpn.is_some() { count += 1; }
        if actions.tailscale_exit_node.is_some() { count += 1; }
        if actions.tailscale_shields.is_some() { count += 1; }
    ...
    pub fn clear_retry_queue(&mut self) {
        debug!("Clearing retry queue with {} pending actions", self.failed_actions.len());
